...
def fetch_page(url):
    ...
    try:
        ...
        headers = {
            'User-Agent': 'SearchWikipedia/1.0 (https://rtimershin.ru/; ruslan.timershin@gmail.com)'
        }
...
def main_script(url1_input, url2_input, rate_limit_rps_input):
    ...
    print(f"Запуск поиска для {url1_input} -> {url2_input}")
    path_1_to_2 = find_path_bfs(url1_input, url2_input, base_wiki_url, max_hops=max_allowed_hops)
    if path_1_to_2:
        print(f"Результат: Путь найден ({len(path_1_to_2) - 1} переходов): {' => '.join(path_1_to_2)}")
    else:
        print(f"Результат: Путь от {url1_input} до {url2_input} не найден за {max_allowed_hops} переходов.")

    print("-" * 40)

    print(f"Запуск поиска для {url2_input} -> {url1_input}")
    path_2_to_1 = find_path_bfs(url2_input, url1_input, base_wiki_url, max_hops=max_allowed_hops)
    if path_2_to_1:
        print(f"Результат: Путь найден ({len(path_2_to_1) - 1} переходов): {' => '.join(path_2_to_1)}")
    else:
        print(f"Результат: Путь от {url2_input} до {url1_input} не найден за {max_allowed_hops} переходов.")
